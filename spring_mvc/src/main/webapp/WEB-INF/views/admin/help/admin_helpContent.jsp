...
<c:if test="${sessGrade == '1' || sessGrade == '2'}">
	<!-- 세부 내용 -->
	<div id="helpShow" class="noticePageStyle">
		<form name="helpDetailFrom" method="post" action="helpDelete.he" onsubmit="return delConfirm()" >
			<p>▣ 공지사항</p>
			<c:forEach items="${list}" var="dto">
				...
				<c:if test="${dto.id == sessId}">	
					<input type="button" value="수정" onclick="helpModify()">	
	...
	<!-- 수정 페이지 -->
	<div id="helpRegForm" class="noticePageStyle" style="display:none" >
		<form name="helpModifyForm" method="post" action="helpModify.ad" onsubmit="return modifyConfirm()" enctype="multipart/form-data" >
			<p>▣ 공지사항</p>
			<c:forEach items="${list}" var="dto">
...
function helpModify() {
	document.getElementById("helpRegForm").style.display="block";
	document.getElementById("helpShow").style.display="none";
}
...
function setThumbnail(event) { 
	...
	reader.onload = function(event) { 
		...
		document.forms["helpModifyForm"]["delCheck"].value = "imgYes";
} 
...
function imageNone() {
	...
	document.forms["helpModifyForm"]["delCheck"].value = "imgNo";
	document.getElementById("changing").value = "changed";
